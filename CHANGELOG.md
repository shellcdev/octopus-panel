...
## v3.1.37 · 硬性规则#4 讨论板逐条强制标注（2026-07-22）

- **问题**：硬性规则 #4 与术语速查仅说讨论板「含观点标签、冲突高亮、违规标注」，未强制逐条套用；观点标签 `[立][怼][让][绕][点][新]` 在 `references/jargon.md`「观点标签」节已完整定义，却未钉进硬规则，实战易出「无标签裸发言」，标签体系形同虚设。
- **修复**：硬性规则 #4 升级为「讨论板实时更新·逐条强制标注」——每条发言开头必须带一个观点标签（歧义判定见 jargon.md「观点标签」节），并按 `references/rules-discussion.md` 实时标冲突高亮 `🔥` 与违规标注 `⚠️/💤`；术语速查同步改为指令式；明令禁止无标签裸发言。仅改指令、不动脚本。
- **审计**：`audit_all` 链接复扫（待跑，预期 exit 0）。
- **README emoji 规范对齐**：emoji 语义表新增 🎨（视觉标记系统，语义规范本体/标记体系总览），`## 🎨 Emoji 语义规范` 标题恢复图标；同步修正 `## 📖 文件导航`→`## 📂 文件导航`（📖 未定义，📂 为已定义「文件/目录」）。符合本规范「新增 emoji 语义须记 CHANGELOG 并更新本规范」规则 #5。

## v3.1.36 · ②.5 停闸显式化（2026-07-22）

- **问题**：②.5 回合结束·停闸（等用户喊「开始」/或先调档·模式·加角色，不得在同条消息进 ③）的语义此前仅在流程草稿中，SKILL.md 正文未落成显式文字，依赖隐含约定。
- **修复**：在 SKILL.md ②.5 工具盘章节后新增「⏸️ 停闸」显式指令段，明确石叔输出工具盘+报当前配置（轮次标注「软默认下限」）后即停闸、不得同条消息进 ③、必须等用户显式开始。
- **审计**：`audit_links.py` 复扫 exit 0（真死链 0）。
