> 📌 版本：v3.1.20 | 更新：2026-07-22 | 维护者：石叔
...
| `references/role-templates.md` | 已验证的角色模板库（10组40个角色） | 生成角色时调用 |
...
- **SKILL.md**（299行）— 总览、硬性/柔性规则、四种讨论模式、🎩轮次滤镜、交互指令、反馈通道
- **README.md**（本文件）— 导航中枢
- **CHANGELOG.md** — 版本变更历史
...
| 文件 | 行数 | 内容 | 何时引用 |
|---|---|---|---|
| `jargon.md` | 550 | 术语通俗释义（含 v3.0 成长系统术语） | 第一次用 |
| `rules-discussion.md` | 714 | 讨论规则（点名/讨论板/小结/收敛/帽子兼容/异常场景1-12） | 讨论进行中 |
| `summary-format.md` | 346 | 总结格式（4种类型）+ 量化判定 + 评分卡 | 总结阶段 |
| `roles-rules.md` | 237 | 角色管理（选择/增加/替补/模板库/标签筛选） | 生成/替换角色时 |
| `rules-collab.md` | 437 | 协作与恢复（多人/L1-L3/归档） | 协作场景 |
| `templates.md` | 829 | 标准模板参考（含🎩帽子轮Prompt模板） | 生成角色前 |
| `role-templates.md` | 1022 | 角色模板库（10组40个角色） | 生成角色时调用 |
| `discussion-examples.md` | 1654 | 19个完整讨论案例（+3边界+1诊断） | 质量核对/格式参照 |
| `role-templates-archive.md` | 14 | 角色模板归档区（退役/合并模板的历史备份） | 查历史模板时 |
| `auto-tag-rules.md` | 140 | 🆕 自动标签规则（8标签+置信度公式+生命周期） | 成长系统自动标签判定（默认关闭） |
| `growth-formula.md` | 156 | 🆕 成长系统量化公式（EXP/等级/衰减/影响力/成就） | 成长数据调参或审计（默认关闭） |
